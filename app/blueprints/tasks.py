...
def admin_panel():
    ...
    setting_prem = SiteSetting.query.filter_by(key='premium_price').first()
    current_premium_price = int(setting_prem.value) if setting_prem else 100 # 100 рублей по умолчанию
    ...
    return render_template('admin.html', 
                           ...
                           current_premium_price=current_premium_price, # Передаем цену ТОПа
                           tickets=tickets)# 🔥 Отмена задачи админом 🔥
...
# 🔥 Рабочая отправка тикета с привязкой к конкретной задаче 🔥
